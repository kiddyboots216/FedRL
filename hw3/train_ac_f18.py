...
class Agent(object):
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        curr_values = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: ob_no})
        next_values = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        adv_n = re_n - curr_values + self.gamma * next_values * (1 - terminal_n)
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            targets = re_n + self.gamma * self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no}) * (1 - terminal_n)
            for _ in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: targets})
