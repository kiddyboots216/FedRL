...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        for i in range(n_layers):
            # layer = tf.layers.dense(layer, size, activation=activation, name="layer{}".format(layer_num))
            # layer_num += 1
            h = activation(h.dot(Ws[i]) + b[i])
...
class Agent(object):
    ...
    def get_weights(self):
        weights = self.sess.run(self.policy_parameters)
